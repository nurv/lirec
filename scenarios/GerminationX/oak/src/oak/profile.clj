...
(defn print-summary
  ...
  (let [name-width (apply max (map (comp count name) (keys profile-summary)))
        fmt-string (str "%" name-width "s  %f  %8d  %8d  %8d  %f%n")]
    ;(printf (.replace fmt-string \d \s)
    ;        "Name" "mean" "min" "max" "count" "sum")
    (doseq [k (sort (keys profile-summary))]
      (let [v (get profile-summary k)]
        (printf fmt-string (name k) (/ (:mean v) 1000000000.0) (:min v) (:max v) (:count v) (/ (:sum v) 1000000000.0))
        ;(println (name k) (/ (:mean v) 1000000000.0))
        ))))
