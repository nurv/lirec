...
Mif.Tree = new Class({
	...
	toggleClick: function(event){
		if(this.mouse.target != 'gadjet' && this.mouse.target != 'name' && this.mouse.target != 'icon') return;
		this.mouse.node.toggle();
	},
	...
	initExpandTo: function(){
	 ...
	 findNodeByUrl: function(urltofind) {
		 var child=this.getrecursiveid(0,this.root.children,urltofind);
		 tree.root.recursive(function(){this.toggle(false);});
		 tree.expandTo(child);
		 tree.select(child);
    },

    getrecursiveid: function(id,children,urltofind)
    {
        for (var i=0,l=children.length;i<l;i++)
        {
                if (children[i].url && urltofind==children[i].url) 
                    {
                        return children[i];
                    }
                    else
                    {
                        ...
                        child=this.getrecursiveid(id,children[i].children,urltofind);
...
Mif.Tree.Draw = {
	...
	getHTML: function(node,html){
		...
		{
			var url;
			if(node.url.indexOf("http://www.stonetrip.com")==-1) url = 'http://www.stonetrip.com/developer/doc/';
			else url='';
			...
			'<div class="mif-tree-node ',(node.isLast() ? 'mif-tree-node-last' : ''),'"'+(node.hidden ? ' style="display:none"' : '')+' id="',prefix,node.UID,'">',
				'<span class="mif-tree-node-wrapper ',node.cls,(node.state.selected ? ' mif-tree-node-selected' : ''),'" uid="',node.UID,'">',
					...
					'<span class="mif-tree-name" uid="',node.UID,'"><a href="',url+node.url,'">',node.name,'</a></span>',
				'</span>',
