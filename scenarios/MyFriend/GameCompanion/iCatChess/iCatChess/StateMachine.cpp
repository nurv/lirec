...
const char *interactive_animations[]={"\\STANDARD\\Modes\\Interactive\\IntrStill.raf",    "\\STANDARD\\Modes\\Interactive\\IntrBlink.raf",
                 "\\STANDARD\\Modes\\Interactive\\IntrLookDown.raf", "\\STANDARD\\Modes\\Interactive\\IntrLookUp.raf",
                 "\\STANDARD\\Modes\\Interactive\\IntrLookLeft.raf", "\\STANDARD\\Modes\\Interactive\\IntrLookRight.raf"};
   ...
   // Check whether a transition needs to be done
	...
	{	
		if (_nextState == SLEEPING_BY_NODDING){
            ...
            animation = "\\STANDARD\\ModeTransition\\Idle2Sleep.raf";
			_currentState = SLEEPING_BY_NODDING;
			_doTransition = false;
			}

			else if (_currentState == INTERACTIVE){
            animation = "\\STANDARD\\ModeTransition\\Interactive2Sleep.raf";
            _currentState = SLEEPING_BY_NODDING;
			_doTransition = false;
			}
         ...
         if (_currentState == SLEEPING_BY_NODDING ||
			 _currentState == SLEEPING_BY_FLASHING){
            animation = "\\STANDARD\\ModeTransition\\Sleep2Idle.raf";
            _currentState = IDLE_LOOK_FRONT;
            _doTransition = false;  
		 }
		else if (_currentState == INTERACTIVE){
            animation = "\\STANDARD\\ModeTransition\\Interactive2Idle.raf";
            _currentState = IDLE_LOOK_FRONT;
            _doTransition = false;
		}
            ...
            _doTransition = false;
			 ...
			 (_currentState == SLEEPING_BY_FLASHING)){
            animation = "\\STANDARD\\ModeTransition\\Sleep2Interactive.raf";
            _currentState = INTERACTIVE;
            _doTransition = false;
			}

		 else if (_currentState == IDLE_LOOK_FRONT){
            animation = "\\STANDARD\\ModeTransition\\Idle2Interactive.raf";
            _currentState = INTERACTIVE;
			GotoState(INTERACTIVE, true);
		 }
