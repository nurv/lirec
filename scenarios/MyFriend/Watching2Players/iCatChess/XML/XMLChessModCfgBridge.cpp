...
const char* XMLChessModCfgBridge::XML_CHILDREN_EMPATHY = "empathy";
...
int XMLChessModCfgBridge::readValuesFromDOMElement(ChessModCfg *cMC, TiXmlElement* pElement) {
	...
	pChild = pElement->FirstChild(XML_CHILDREN_EMPATHY);

	cMC->setEmpathy(atoi(pChild->ToElement()->Attribute(XML_ATTRIBUTE_VALUE)));
