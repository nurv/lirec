...
//always consider the companion's side
int ChessEngine::genHeurValue(int thinkingTime){
	think(thinkingTime);
	if(side == 0){ //companion's side
		return -retLastX();
	}else{
		return retLastX();
	}
...
char* ChessEngine::play(int thinkingTime){

		Console::WriteLine("\n********\n*******\nINVOCACAO DA FUNCAO PLAY\nISTO NAO DEVIA ACONTECER\n********\n******");
...
char ChessEngine::getPieceFromPosition(char column, int line){
	int columnInt= column -97;
	if (color[columnInt + (8-line) *8]== LIGHT)
		return piece_char[piece[columnInt + (8 - line) *8]];
	else
		return piece_char[piece[columnInt + (8- line) *8]];
}
