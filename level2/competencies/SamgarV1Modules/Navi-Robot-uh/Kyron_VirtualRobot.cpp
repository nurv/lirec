...
public :
	...
	if(NewData){WhereInList=0;PrecBottle=MoveBottle;}
			...
			if(myRobot->isHeadingDone(1) && myRobot->isMoveDone(5) && WhereInList<PrecBottle.size() && InTransit == false)
			{
				myDesired.setDeltaHeading(PrecBottle.get(WhereInList).asDouble());
				WhereInList++;
				//myDesired.move(PrecBottle.get(WhereInList).asDouble());
				WhereInList++;
			}
...
public :
	...
	//	speed=speed/2;
		...
		if(InTransit==true)
		{
			...
			//	myDesired.setRotVel(currentheadingchange,1);
				
			//	if(currentheadingchange<90){}
			//	else						{myDesired.setVel(0,1);}
			//	printf("speed : %f \n",(speed/100)*proportoinalcontrol);
			//	myDesired.
			//	myDesired
		}
		else
