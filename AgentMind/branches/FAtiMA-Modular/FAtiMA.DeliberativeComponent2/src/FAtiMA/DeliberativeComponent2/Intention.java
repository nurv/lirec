...
public class Intention implements Serializable {
	...
	private ArrayList<Plan> _solutions;
	...
	public Intention(AgentModel am, ActivePursuitGoal g) {
		...
		_solutions = new ArrayList<Plan>();
		_fearEmotionID = null;
		_hopeEmotionID = null;
		_strongCommitment = false;
		_appraisalFrame = new AppraisalFrame(g.GetActivationEvent());
		_appraisalFrame.SetAppraisalVariable(DeliberativeComponent2.NAME, (short) 7, OCCAppraisalVariables.GOALSTATUS.name(), OCCAffectDerivationComponent.GOALUNCONFIRMED);
	...
	public void AddSolution(Plan p)
	{
		_solutions.add(p);
	}
	
	public ArrayList<Plan> GetSolutions()
	{
		return _solutions;
	}
	
	public boolean containsEquivalentSolution(Plan p)
	{
		for(Plan solution : this._solutions)
		{
			//simplified test for now
			if(solution.toString().equals(p.toString()))
			{
				return true;
			}
		}
	...
	public Plan getBestSolution(AgentModel am)
	{
		if(_solutions.size() == 0)
		{
			return null;
		}
		else return _solutions.get(0);
	}
	...
	public float GetProbability(AgentModel am) {
		...
		DeliberativeComponent2 dp = (DeliberativeComponent2) am.getComponent(DeliberativeComponent2.NAME);
	...
	public void CheckLinks(AgentModel am) {
		...
		//in order to be more efficient we only check links for complete solutions
	    ListIterator<Plan> li;
	    li = _solutions.listIterator();
	...
	public void ProcessIntentionActivation(AgentModel am) 
	{
	    ...
	    _appraisalFrame.SetAppraisalVariable(DeliberativeComponent2.NAME, (short)7, OCCAppraisalVariables.SUCCESSPROBABILITY.name(), probability);
	    _appraisalFrame.SetAppraisalVariable(DeliberativeComponent2.NAME, (short)7, OCCAppraisalVariables.FAILUREPROBABILITY.name(), 1-probability);
	    _appraisalFrame.SetAppraisalVariable(DeliberativeComponent2.NAME, (short)7, OCCAppraisalVariables.GOALCONDUCIVENESS.name(), goalConduciveness);
	    _appraisalFrame.SetAppraisalVariable(DeliberativeComponent2.NAME, (short)7, OCCAppraisalVariables.GOALSTATUS.name(), OCCAffectDerivationComponent.GOALUNCONFIRMED);
	...
	{	
	    ...
	    af.SetAppraisalVariable(DeliberativeComponent2.NAME,(short)8,OCCAppraisalVariables.GOALSTATUS.name(), OCCAffectDerivationComponent.GOALDISCONFIRMED);
	    af.SetAppraisalVariable(DeliberativeComponent2.NAME, (short)8,OCCAppraisalVariables.GOALCONDUCIVENESS.name(), dp.getUtilityStrategy().getUtility(am, _goal));
	...
	public void ProcessIntentionSuccess(AgentModel am) 
	{
	    ...
	    am.getMemory().getEpisodicMemory().StoreAction(am.getMemory(), e);
	    AppraisalFrame af = new AppraisalFrame(e);
	    af.SetAppraisalVariable(DeliberativeComponent2.NAME,(short)8,OCCAppraisalVariables.GOALSTATUS.name(), OCCAffectDerivationComponent.GOALCONFIRMED);
	    af.SetAppraisalVariable(DeliberativeComponent2.NAME, (short)8,OCCAppraisalVariables.GOALCONDUCIVENESS.name(), dp.getUtilityStrategy().getUtility(am, _goal));
	    am.updateEmotions(af);
