...
import FAtiMA.DeliberativeComponent2.display.GoalsPanel;
import FAtiMA.DeliberativeComponent2.parsers.DeliberativeLoaderHandler;
import FAtiMA.DeliberativeComponent2.strategies.DefaultStrategy;
import FAtiMA.DeliberativeComponent2.strategies.IActionFailureStrategy;
import FAtiMA.DeliberativeComponent2.strategies.IActionSuccessStrategy;
import FAtiMA.DeliberativeComponent2.strategies.IExpectedUtilityStrategy;
import FAtiMA.DeliberativeComponent2.strategies.IGetUtilityForOthers;
import FAtiMA.DeliberativeComponent2.strategies.IGoalFailureStrategy;
import FAtiMA.DeliberativeComponent2.strategies.IGoalSuccessStrategy;
 ...
 * @author João Dias
 */
public class DeliberativeComponent2 implements Serializable, IComponent,
	...
	private static final long PLANNING_TIME = 3000;
	...
	private ValuedAction getSelectedAction() {
		...
		return new ValuedAction(DeliberativeComponent2.NAME,_selectedAction.getName(), _selectedActionEmotion);
	}
	...
	private Intention deliberation(AgentModel am)
	{
		Intention i;
	...
	public ValuedAction actionSelection(AgentModel am) {
		...
		_currentIntention = deliberation(am);

		if(_currentIntention != null)
		{
			//means-ends reasoning
			if(_currentIntention.getPlanningTime() <= PLANNING_TIME && _currentIntention.GetSolutions().size() <0)
			{
				p = _planner.ThinkAbout(am, this, _currentIntention);
				if(p!=null)
				{
					//by updating the plan, we make sure the plan is not unnecessarily bigger
					//than it should, however, we may have duplicate plans
					p.UpdatePlan(am);
					if(!_currentIntention.containsEquivalentSolution(p))
					{
						AgentLogger.GetInstance().logAndPrint("Found solution: " + p);
						_currentIntention.AddSolution(p);
					}
					_currentIntention.RemovePlan(p);
				}
			}
			else
			{
				...
				if(_currentIntention.GetSolutions().size()==0)
				{
					//no solutions found
					return null;
				}
				//final deliberation
				//selecting between alternative solutions
				_selectedPlan = _currentIntention.getBestSolution(am);
				
				if(_selectedPlan != null && !_selectedPlan.isSolution())
				{
					//a solution stopped being a valid solution, likely because the world 
					//has changed. This means we need to replan this particular solution
					_currentIntention.GetSolutions().remove(_selectedPlan);
					_currentIntention.AddPlan(_selectedPlan);
					_currentIntention.restartPlanning();
					return null;
				}
				prepareAction(am);
	...
	private void prepareAction(AgentModel am)
	{
		...
		if (_selectedPlan != null
				&& _selectedPlan.isSolution()
				&& _selectedPlan.getSteps().size() == 0) 
		{
			removeIntention(_currentIntention);
			if(_currentIntention.IsStrongCommitment())
			{
				for (IGoalSuccessStrategy s : _goalSuccessStrategies) 
				{
					s.perceiveGoalSuccess(am, _currentIntention.getGoal());
				}
				_currentIntention.ProcessIntentionSuccess(am);
				cancelAction(am);
			}
		...
		if (_actionMonitor == null && _selectedPlan != null) {
			...
			if (copingAction != null) 
			{
				if (!_currentIntention.IsStrongCommitment()) 
				{
					...
					_currentIntention.SetStrongCommitment(am);
					_currentIntention.ProcessIntentionActivation(am);
				}
				...
				{
					fear = _currentIntention.GetFear(am.getEmotionalState());
					hope = _currentIntention.GetHope(am.getEmotionalState());
					if (hope != null) 
					{
						if (fear != null) 
						{
					...
					AgentLogger.GetInstance().log("Selecting Action: " + _selectedAction.toString() + "from plan:"
					...
					{
						eff = (Effect) li.next();
						if (eff.isGrounded()) am.getMemory().getSemanticMemory().Tell(true,eff.GetEffect().getName(),eff.GetEffect().getValue().toString());
					}
					this.checkLinks(am);
				} 
				else 
			...
			else 
			{
				...
				// must be removed
				_currentIntention.RemovePlan(_selectedPlan);
	...
	public IComponent createModelOfOther() {
		return new DeliberativeComponent2();
	}
	...
	@Override
	public String name() {
		return DeliberativeComponent2.NAME;
	}
