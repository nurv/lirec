...
This code is based on the face detection code provided by OpenCV and allows for the prediction of whether the user is staying still, approaching the camera or withdrawing.

In the function detect_and_draw of the file face.cpp, the face is detected and the area of the face bounding box is returned for each frame.
The values of the area are stored and used for the prediction of the type of movement (staying still, approaching the camera or withdrawing) using the functions of the class "DistanceCheck".
The area of the face bounding box in the current frame is compared with the values of the area in a temporal window (currently set to size 4 frames) preceding it.
