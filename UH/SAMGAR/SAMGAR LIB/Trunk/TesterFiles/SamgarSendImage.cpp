...
int main() {
	...
	SamgarModule MyFirstTest("Module1","behaviour","happy","Cont"); // Cant have spaces or underscores
	// then we setup the image port
	MyFirstTest.SetupImagePort("VideoOut");

    SamgarModule MyFirstTest2("Module2","behaviour","happy","Cont"); // Cant have spaces or underscores
	MyFirstTest2.SetupImagePort("VideoIn");

	// can use samgarkey to connect ports but for this demo we'll ignore it and do it manually (this shouldnt be done)
	Network::connect("/Port_Module1_VideoOut_Yarp","/Port_Module2_VideoIn_Yarp","udp");
	Network::connect("/Port_Module2_VideoIn_Yarp","/Port_Module1_VideoOut_Yarp","udp");
