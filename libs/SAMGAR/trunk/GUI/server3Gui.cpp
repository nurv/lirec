		     ...
		     QWidget *widget)
{
   ...
   if(this->isSelected())
      painter->drawPixmap(target2,QPixmap("Sprites.png"), source2);
   else
      painter->drawPixmap(target2,QPixmap("Sprites.png"), source2);
		       ...
		       const QStyleOptionGraphicsItem *option,QWidget *widget)
{
   ...
   if(place>360)
      place=0;
   painter->drawPixmap(target2,QPixmap("Rotate.png"),Asource);
   painter->restore();

   if(this->isSelected())
   {
      if(online)
	 painter->drawPixmap(target,QPixmap("Sprites.png"), Ssource);
      else	  
	 painter->drawPixmap(target,QPixmap("Sprites.png"), SOsource);
   }
   else
   {
      if(online)
	 painter->drawPixmap(target,QPixmap("Sprites.png"), Nsource);
      else
	 painter->drawPixmap(target,QPixmap("Sprites.png"), Osource);
   }
...
//////////////////////////////////////////////////////////////////////
  ...
  void ToolWindow::CheckconXX(int connectthem)
  {
	 for(int hh =0;hh<maxconns ;hh++)
	{
      if(SamCons[hh].GetValid())
	  {
		  bool iscon1 = Network::isConnected(SamCons[hh].GetDad()->GetRealName().c_str(),SamCons[hh].GetMom()->GetRealName().c_str(),true);
		  bool iscon2 = Network::isConnected(SamCons[hh].GetMom()->GetRealName().c_str(),SamCons[hh].GetDad()->GetRealName().c_str(),true);
		  if(iscon1&&iscon2&&connectthem==0){SamCons[hh].SetConnected(true);}
		  else if(connectthem==0)			{SamCons[hh].SetConnected(false);}
		  else if(iscon1&&iscon2)			{SamCons[hh].SetConnected(true);}
		  else
		  {
			string connType;
			switch (SamCons[hh].GetConn()) 
			{
			case SAMdef::udp :  connType = "udp"; break;
			case SAMdef::tcp :  connType = "tcp"; break;
			case SAMdef::local: connType = "local"; break;
			...
			Bottle cmd1,reply;
			cmd1.clear();
			cmd1.addString("subscribe");
			cmd1.addString(SamCons[hh].GetMom()->GetRealName().c_str());
			cmd1.addString(SamCons[hh].GetDad()->GetRealName().c_str());
			cmd1.addString(connType.c_str());
			Network::write(Network::getNameServerContact(),cmd1,reply);
			cmd1.clear();
			cmd1.addString("subscribe");
			cmd1.addString(SamCons[hh].GetDad()->GetRealName().c_str());
			cmd1.addString(SamCons[hh].GetMom()->GetRealName().c_str());
  ...
  void ToolWindow::ReconM(void)
  {
CheckconXX(1);

  }
 void ToolWindow::CheckconM(void)
  {
CheckconXX(0);
...
ToolWindow::ToolWindow()
{
   ...
   CheckCon =  new QPushButton("Check all connections",this);
   ReCon =  new QPushButton("Reconnect all",this);
   ...
   BLayout->addWidget(CheckCon,0,2); 
   BLayout->addWidget(ReCon,0,3);
   ...
   setLayout(mainLayout);
	...
	connect(CheckCon, SIGNAL(clicked()),this, SLOT(CheckconM()));
	connect( ReCon, SIGNAL(clicked()),this, SLOT(ReconM()));
