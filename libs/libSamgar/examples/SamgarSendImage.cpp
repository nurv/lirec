...
// To use samgar you only need to attach the SamgarLib and the
// SamgarMainClass header
...
int main() {
  ...
  Samgar::SamgarModule CameraSend("CameraSend","Image",
				  "Camera",Samgar::ModeRun); 
  // Cant have spaces or underscores
  // then we setup the image port
  CameraSend.SetupImagePort("VideoOut");
  // OCV get a device
  ...
  while( 1 ) {
      if (CameraSend.getCurrentState() == Samgar::StateRunning)
      {
          ...
          // send the frame
          CameraSend.SendPictureOCVNative(frame);
      }
      if( (cvWaitKey(10) & 255) == 27 ) break;
      CameraSend.SucceedFail(true,0);
  }
