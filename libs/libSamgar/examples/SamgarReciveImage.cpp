...
//**********************************************************************
//
//Program : example reciving images with SAMGAR
//Date	: 24 Aug 2009
//Author  : K Du Casse
//
//A simple program that shows how to Recive images with SAMGAR
//
//**********************************************************************
...
int main() {
  Network yarp;
  Samgar::SamgarModule CameraRecive("CameraRecive","PersonFinder",
				    "Camera",Samgar::ModeRun); 
  // Cant have spaces or underscores
  CameraRecive.SetupImagePort("VideoIn");
  cvNamedWindow( "copy", CV_WINDOW_AUTOSIZE );
  while( 1 )
  {
      if (CameraRecive.getCurrentState() == Samgar::StateRunning)
      {
          IplImage *frame2 = CameraRecive.RecivePictureOCVNative();
          if(frame2!=false) {// if there is no image available (one hasn't
              // been sent) then the image will be false
              cvShowImage( "copy", frame2 ); // if its not false then display it.
          }          
      }
      if( (cvWaitKey(10) & 255) == 27 ) break;
      CameraRecive.SucceedFail(true,0);
  }
