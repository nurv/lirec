...
void PortIcon::CreateDeleateCons(void)
{
  ...
  for(int bb =0;bb<maxconns;bb++)
  {
    ...
    if(TempConnect.GetDad()==SamCons[bb].GetMom() && 
  ...
  if(NewonetoAdd) // if its a new one 
  {
	...
	//	yarphelp.connect
	string conny;
	if(SamCons[bb].GetConn()==0)
	  conny="udp";
	else if(SamCons[bb].GetConn()==1)
	  conny="tcp";
	else if(SamCons[bb].GetConn()==2)
	  conny="local";
	else conny="shmem";
		       ...
		       const QStyleOptionGraphicsItem *option,QWidget *widget)
{
  ...
  {
    if(online)
      painter->drawPixmap(target,myiconpic, Ssource);
    else	  
      painter->drawPixmap(target,myiconpic, SOsource);
  }
  else
  {
    if(online)
      painter->drawPixmap(target,myiconpic, Nsource);
    else
      painter->drawPixmap(target,myiconpic, Osource);
  }
  ...
  */
  ...
  {	
    if(!SamCons[uu].GetValid()&&gotmom&&gotdad) // empty space
    {
      ...
      string conny;
      if(SamCons[uu].GetConn()==0)
	conny="udp";
      else if(SamCons[uu].GetConn()==1)
	conny="tcp";
      else if(SamCons[uu].GetConn()==2)
	conny="local";
      else 
	conny="shmem";
...
void MyConnectionView::updateline(void)
{
  ...
  for(int cc=0;cc<100;cc++)
  {
	...
	mypenforc.setStyle(Qt::SolidLine);
      else 
	mypenforc.setStyle(Qt::DashLine);
  ...
  if(Connecting == 1)
  {
    ...
    myline1->setLine(connstartx+Xshift, connstarty+YoffSet,
		     connstartx+Xshift, connstarty+YoffSet+(ydiff/2));// straight down but only half value
    myline2->setLine(connstartx+Xshift, connstarty+YoffSet+(ydiff/2),
		     connstartx+Xshift+xdiff, connstarty+YoffSet+(ydiff/2));
    myline3->setLine(connstartx+Xshift+xdiff, connstarty+YoffSet+(ydiff/2),
		     connstartx+Xshift+xdiff, connstarty+YoffSet+(ydiff));
