...
VideoGet::VideoGet(std::string name): SamClass(name)
{
}

void VideoGet::SamInit()
{
   newPort(&videoRecive, "In"); // add new port  
   StartModule();	
   puts("started video recieve");
}

void VideoGet::SamIter()
{
}

yarp::sig::ImageOf<yarp::sig::PixelBgr>* VideoGet::getImagePtr()
{
   return videoRecive.read(true); // true means it'll wait for image
}

int main( void)
{
  VideoGet MyGetMod("/VideoG");     // create instance
  MyGetMod.SamInit();  // do samgar only stuff in its own innit, keeps the rest of the code clear from clutter
  char Vid2[] = "After";
  IplImage * frm;
  ...
  {
     yarp::sig::ImageOf<yarp::sig::PixelBgr> *yarpImage2 = MyGetMod.getImagePtr(); // true means it'll wait for image
